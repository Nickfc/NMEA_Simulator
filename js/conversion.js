class Conversion {
  ...
  interpolateRoutePoints(totalRouteTime, frequency) {
    ...
    for (let i = 1; i < this.routePoints.length; i++) {
      remainingDistance += GeoUtils.haversineDistance(this.routePoints[i - 1], this.routePoints[i]);
    }

    for (let i = 1; i < this.routePoints.length; i++) {
      const segmentDistance = GeoUtils.haversineDistance(this.routePoints[i - 1], this.routePoints[i]);
